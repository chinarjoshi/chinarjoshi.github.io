...
Every* function can be approximated, from *f(x) =* sin(*x*) to driving cars. Different approaches to approximation expose different tradeoffs, but the most important of which are that
  1. Simple models can't arbitrarily approximate complex functions, and
  2. Complex models require exponentially more resources.
...
While approximation is theoretically grounded, so is time travel and Dyson spheres. In practice, that second challenge—the *curse of dimensionality*—is the hungry, unstoppable thorn in our innocent dreams. Yet, we thanks to the sweat of engineers across the stack, our resources <small>(data & compute)</small> have kept pace with our dreams: Moore's law. Specifically, this is the trend that the size of transistors is exponentially decreasing, allowing our computers to do more, faster. Though the original Moore's law is obsolete because we've hit the limit before Schrödinger gets upset, a so-called *"Moore's law 2.0,"* has no end in sight, or increasing *parallel* processing units on-and-between chips. Lucky for us <small>(and Jensen Huang)</small>, training neural networks is a *spectacularly* parallelizable problem, making a happy marriage with the GPU, an army of 7 year olds who can do no more than multiply and add numbers.

But that we have the power, how far can we push it? What if that cost function is like *h(x) = x is not consciousness*? We actually have no clue what that training could produce, but it depends on whether consciousness is physical, which is a *huge* but necessary detour from our compute conversation. Neuroscientists have identified parts of the brain correlated with conscious experience, but we have zero clues *how* or *why* it works. We have some ad-hoc ideas though. An age-old benchmark to discriminate conscious behavior is the *Turing Test*, where a model tries to fool a human into thinking it's also human. 

That being said, today's LLMs are trying to model a significantly simpler function than our brain: natural language. Language production might really be a corollary of consciousness, but with high bias since its a simplification. Yet, as celebrities could tell you, there are many consequences to imitation. Anyone can generate defamatory fake media, and there is quite literally *nothing* that regulators can do about it. These models are just lists of numbers—trivial to distribute, impossible to trace. If these exponential trends in data and compute are sustained, then bad actors **will** train their own models outside Big Tech warehouses. Therefore, regulators can dull, but never declaw generative AI due to Moore's law.

Honestly, the bigger question isn't *can* we do this, but *why would we want to in the first place?*

Obviously, machines can already break boulders, drive cars, and swoon crushes better than most Sapiens. And while it's good fun for mathmeticians to decipher the universe and for engineers to build cool things, why is the rest of the world trapped in our rat race of pushing metrics? 

*"Can we?"* is a technical question, and the answer is almost always *"yes."* 

*"Should we?"* is a moral question, and its answer is usually *"no!"* 
...
Either way, I'm just excited to witness such a provocative time for our species.
